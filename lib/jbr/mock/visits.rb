module Jbr
  ...
  class Mock::Visits < Visits
    ...
    # @return [Company::Visit, nil] visit the app listed under that ID, nil where it listed none.
    def find(id)
      node = Jbr.mock.visits.to_a.find { |visit| visit[:id] == id }
      Company::Visit.new node: node if node
    end
  ...
  private
    ...
    def walk(_statement)
      Enumerator.new do |yielder|
        selected.each { |node| yielder << Company::Visit.new(node: node) }
      end
    end
